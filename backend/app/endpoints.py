...
class FinancialAgent(Chain):
    ...
    def _call(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = self.llm.invoke(prompt.format(document_content=inputs["document"])).content
        ...
        try:
            ...
            return {
                "safety_score": 0,
                "comments": f"Error parsing response: {str(e)}"
            }
...
class RouterAgent:
    ...
    async def analyze_document(self, content: str, doc_type: DocumentType = None) -> AgentScore:
        if doc_type is None:
            doc_type = self.route_document(content)
        
        agent = self.financial_agent if doc_type == DocumentType.FINANCIAL else self.business_agent
...
async def perform_due_diligence(
    ...
    is_safe = avg_score >= 60 and all(score.safety_score >= 50 for score in agent_scores)
