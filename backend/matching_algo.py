...
def generate_investor_text_for_embedding(investor: Dict[str, Any]) -> str:
    ...
    parts.append(f"Preferred Industries: {', '.join(investor.get('preferred_industries', []))}")
    ...
    parts.append(f"Preferred Locations: {', '.join(investor.get('preferred_locations', []))}")
...
def calculate_match_score(
    ...
    score = 0.0
    max_possible_score = sum([weight for weight in MATCH_WEIGHTS.values() if weight > 0])
    ...
    if company_rev_stage:
        ...
        early_investor = any(s in ['seed', 'pre-seed'] for s in investor_data.get('preferred_stages', []))
...
def find_matches_for_company(
    ...
    for investor in investors_data:
        ...
        details={
            ...
            "exit_timeline": f"{investor.get('exit_timeline_years')} years" if investor.get('exit_timeline_years') else "Not specified"
        }
    ))
