...
async def display_investor_info(investor):
    ...
    print(
        f"Investment Range: ${investor.min_investment_usd:,.2f} - "
        + (
            f"${investor.max_investment_usd:,.2f}"
            if investor.max_investment_usd
            else "No upper limit"
        )
    )
...
async def run_demo():
    ...
    try:
        ...
        print(
            f"Found {len(companies)} companies and {len(investors)} investors in the database"
        )
        ...
        matches = find_matches_for_company(
            target_company.id, companies, investors, top_n=3
        )
        ...
        matches = find_matches_for_investor(
            target_investor.id, companies, investors, top_n=3
        )
