...
from models import Company, Investor, RevenueStage, BusinessModel, ExitStrategy, FocusArea, FounderType, RiskAppetite, TimeHorizon
...
INDUSTRY_SUB_INDUSTRIES = {
    ...
    "AI": ["Machine Learning", "Natural Language Processing", "Computer Vision", "Robotics", "Predictive Analytics"],
    "Biotech": ["Genomics", "Pharmaceuticals", "Diagnostic Tools", "Gene Therapy", "Bioinformatics"],
    "HealthTech": ["Telemedicine", "Health Monitoring", "Medical Devices", "Health Records", "Digital Therapeutics"],
    "CleanTech": ["Renewable Energy", "Energy Storage", "Carbon Capture", "Smart Grid", "Water Tech"],
    "Fintech": ["Payment Processing", "InsurTech", "Blockchain", "Personal Finance", "RegTech"],
    "Mobility": ["Electric Vehicles", "Ride Sharing", "Logistics", "Urban Transport", "Autonomous Vehicles"],
    "AgriTech": ["Precision Agriculture", "Smart Farming", "Crop Monitoring", "Supply Chain", "Vertical Farming"],
    "Sustainability": ["Circular Economy", "Waste Management", "Eco Materials", "Conservation", "Carbon Footprint"],
    "EdTech": ["E-Learning", "Educational Games", "Language Learning", "School Management", "Career Development"],
    "Aerospace": ["Satellite Tech", "Space Tourism", "Propulsion Systems", "Drone Technology", "Space Mining"],
    "MedTech": ["Medical Imaging", "Surgery Tech", "Patient Monitoring", "Diagnostics", "Rehabilitation"],
    "Cybersecurity": ["Threat Detection", "Data Protection", "Identity Management", "Network Security", "Cryptography"],
    "Big Data": ["Data Analytics", "Business Intelligence", "Data Visualization", "Real-time Processing", "Data Storage"],
    "Quantum Computing": ["Quantum Hardware", "Quantum Algorithms", "Quantum Software", "Quantum Security", "Quantum Sensing"],
    "FoodTech": ["Alternative Proteins", "Food Delivery", "Ghost Kitchens", "Food Processing", "Precision Fermentation"],
    "LegalTech": ["Contract Analysis", "Legal Research", "Compliance", "E-Discovery", "IP Management"],
    "XR/VR": ["Virtual Reality", "Augmented Reality", "Mixed Reality", "Immersive Training", "Digital Twins"]
}
    ...
    for row in rows:
        ...
        if 'sub_industries' not in company_data or not company_data['sub_industries']:
            ...
            if possible_subs:
                ...
                updated_fields['sub_industries'] = json.dumps(random.sample(possible_subs, count))
        ...
        if 'revenue_stage' not in company_data or not company_data['revenue_stage']:
            ...
            possible_stages = STAGE_REVENUE_MAPPING.get(stage, [RevenueStage.EARLY_REVENUE])
        ...
        if 'business_model' not in company_data or not company_data['business_model']:
            industry = company_data.get('industry', '')
            possible_models = INDUSTRY_BUSINESS_MODEL.get(industry, [BusinessModel.SAAS])
        ...
        if 'exit_strategy' not in company_data or not company_data['exit_strategy']:
            ...
            if valuation > 40_000_000:
                ...
                updated_fields['exit_strategy'] = random.choice([ExitStrategy.ACQUISITION.value, ExitStrategy.IPO.value])
            else:
        ...
        if 'focus_areas' not in company_data or not company_data['focus_areas'] or company_data['focus_areas'] == '[]':
            ...
            if focus_count > 0:
                ...
                if "Sustainability" in company_data.get('industry', '') or "Clean" in company_data.get('industry', ''):
                    sustainability_focus = ["sustainability", "esg", "climate_tech"]
                    chosen = random.sample(sustainability_focus, min(focus_count, len(sustainability_focus)))
        ...
        if 'founder_types' not in company_data or not company_data['founder_types'] or company_data['founder_types'] == '[]':
            # Get industry-specific founder tendencies
            industry = company_data.get('industry', '')
            founder_tendencies = INDUSTRY_FOUNDER_TENDENCIES.get(industry, ["TECHNICAL", "FIRST_TIME"])
            
            # Add 1-2 founder types, ensuring at least one from the industry tendency if possible
            count = random.randint(1, 2)
            if count == 1:
                updated_fields['founder_types'] = json.dumps([random.choice(founder_tendencies)])
            else:
        ...
        if 'time_horizon' not in company_data or not company_data['time_horizon']:
            ...
            if exit_strategy == ExitStrategy.IPO.value:
                ...
                updated_fields['time_horizon'] = random.choice([TimeHorizon.MEDIUM_TERM.value, TimeHorizon.LONG_TERM.value])
            else:
                # Later stage can have shorter horizons
                updated_fields['time_horizon'] = random.choice([t.value for t in TimeHorizon])
        ...
        if 'esg_focus' not in company_data or company_data['esg_focus'] is None:
            ...
            updated_fields['esg_focus'] = 1 if random.randint(1, 100) <= likelihood else 0
        ...
        if updated_fields:
            ...
            if company:
                ...
                if embedding is not None:
                    ...
                    await db.db.execute("UPDATE company SET embedding = ? WHERE id = ?", (embedding_json, company_id))
                    await db.db.commit()
                    print(f"Updated company: {company.name}")
                else:
                    print(f"Failed to generate new embedding for company: {company.name}")
            else:
...
async def update_investors_direct_db(db: Database) -> None:
    ...
    for row in rows:
        ...
        if 'excluded_industries' not in investor_data or not investor_data['excluded_industries'] or investor_data['excluded_industries'] == '[]':
            # Common industries that might be excluded
            common_exclusions = ["Gambling", "Tobacco", "Weapons", "Adult Entertainment"]
            
            # Impact investors more likely to exclude fossil fuels
            if "Impact" in investor_data.get('investor_type', ''):
                exclusions = ["Fossil Fuels", "Mining"] + random.sample(common_exclusions, 1)
            else:
        ...
        if 'business_model_focus' not in investor_data or not investor_data['business_model_focus'] or investor_data['business_model_focus'] == '[]':
            # Get preferred industries - safely parse the JSON
            try:
                preferred_industries_str = investor_data.get('preferred_industries', '[]') 
                if preferred_industries_str and preferred_industries_str != 'null' and preferred_industries_str != 'None':
            ...
            if not business_models:
                ...
                business_models = [BusinessModel.SAAS.value, BusinessModel.SUBSCRIPTION.value]
        ...
        if 'exit_timeline_years' not in investor_data or not investor_data['exit_timeline_years']:
            ...
            try:
                ...
                if preferred_stages_str and preferred_stages_str != 'null' and preferred_stages_str != 'None':
            ...
            elif any(stage in ["Series B", "Series C", "Growth"] for stage in preferred_stages):
        ...
        if 'preferred_focus_areas' not in investor_data or not investor_data['preferred_focus_areas'] or investor_data['preferred_focus_areas'] == '[]':
            # Impact investors focus on sustainability
            if "Impact" in investor_data.get('investor_type', '') or investor_data.get('esg_mandate', 0) == 1:
                focus_areas = ["sustainability", "esg", "climate_tech", "social_impact"]
                count = random.randint(2, 4)
                updated_fields['preferred_focus_areas'] = json.dumps(random.sample(focus_areas, min(count, len(focus_areas))))
            else:
                # Other investors may have some focus areas
                all_focus_areas = [area.value for area in FocusArea]
                count = random.randint(0, 3)
                if count > 0:
                    updated_fields['preferred_focus_areas'] = json.dumps(random.sample(all_focus_areas, count))
                else:
                    updated_fields['preferred_focus_areas'] = json.dumps([])
        
        # Add preferred_founder_types if missing
        if 'preferred_founder_types' not in investor_data or not investor_data['preferred_founder_types'] or investor_data['preferred_founder_types'] == '[]':
            ...
            if "Impact" in investor_data.get('investor_type', '') or investor_data.get('esg_mandate', 0) == 1:
                diversity_focus = ["female_founders", "underrepresented_groups", "young_founders"]
                count = random.randint(1, 2)
                updated_fields['preferred_founder_types'] = json.dumps(random.sample(diversity_focus, min(count, len(diversity_focus))))
            else:
                # Other investors may have some preferences or none
                count = random.randint(0, 2)
                if count > 0:
                    updated_fields['preferred_founder_types'] = json.dumps(random.sample(all_founder_types, count))
                else:
        ...
        if 'risk_appetite' not in investor_data or not investor_data['risk_appetite']:
            ...
            if "Angel" in investor_type or all(stage in ["Pre-Seed", "Seed"] for stage in preferred_stages if stage):
                # Early stage investors typically have higher risk appetite
                risk_options = [RiskAppetite.AGGRESSIVE, RiskAppetite.VERY_AGGRESSIVE]
            elif any(stage in ["Series C", "Growth"] for stage in preferred_stages if stage):
        ...
        if 'preferred_time_horizon' not in investor_data or not investor_data['preferred_time_horizon'] or investor_data['preferred_time_horizon'] == '[]':
        ...
        if updated_fields:
            ...
            if investor:
                ...
                if embedding is not None:
                    # Update only the embedding
                    embedding_json = json.dumps(embedding.tolist())
                    await db.db.execute("UPDATE investors SET embedding = ? WHERE id = ?", (embedding_json, investor_id))
                    await db.db.commit()
                    print(f"Updated investor: {investor.name}")
                else:
                    print(f"Failed to generate new embedding for investor: {investor.name}")
            else:
...
async def get_investor_by_id(db: Database, investor_id: str) -> Optional[Investor]:
    ...
    for field in ['preferred_industries', 'preferred_stages', 'preferred_locations', 
                ...
                'excluded_industries', 'business_model_focus']:
...
async def update_database_schema(db: Database) -> None:
    ...
    try:
        ...
        await db.db.execute("CREATE TABLE IF NOT EXISTS investors (id TEXT PRIMARY KEY)")
    ...
    try:
        ...
        await db.db.execute("ALTER TABLE company ADD COLUMN sub_industries TEXT DEFAULT '[]'")
    ...
    try:
        ...
        await db.db.execute("ALTER TABLE company ADD COLUMN esg_focus INTEGER DEFAULT 0")
    ...
    try:
        ...
        await db.db.execute("ALTER TABLE investors ADD COLUMN excluded_industries TEXT DEFAULT '[]'")
    ...
    try:
        ...
        await db.db.execute("ALTER TABLE investors ADD COLUMN business_model_focus TEXT DEFAULT '[]'")
    ...
    try:
        ...
        await db.db.execute("ALTER TABLE investors ADD COLUMN esg_mandate INTEGER DEFAULT 0")
    ...
    try:
        ...
        await db.db.execute("ALTER TABLE investors ADD COLUMN exit_timeline_years INTEGER")
